...
//	DA          cda;
	...
	//	DA          cda   = user[bi].cda;
		...
		PetscMalloc(NUM_OF_PARTICLE*sizeof(PetscReal), &acceleration_x);
                PetscMalloc(NUM_OF_PARTICLE*sizeof(PetscReal), &acceleration_y);
		PetscMalloc(NUM_OF_PARTICLE*sizeof(PetscReal), &acceleration_z);
		...
		PetscMalloc(NUM_OF_PARTICLE*sizeof(PetscReal), &(user[bi].Fbx));
		PetscMalloc(NUM_OF_PARTICLE*sizeof(PetscReal), &(user[bi].Fby));
		PetscMalloc(NUM_OF_PARTICLE*sizeof(PetscReal), &(user[bi].Fbz));
		...
		for (particle=0; particle<NUM_OF_PARTICLE; particle++) {
			...
			PetscInt  lp=0.0,up=1000.0;
                        PetscReal random_vel_x = (rand() % (up - lp + 1) + lp)*0.0005;
                        PetscReal random_vel_y = (rand() % (up - lp + 1) + lp)*0.0005;
                        PetscReal random_vel_z = (rand() % (up - lp + 1) + lp)*0.00001;
			...
			/*for (ipini=lxs+1; ipini<lxe; ipini++) {
				for(jpini=lys+1; jpini<lye;jpini++){
					for(kpini=lzs+1; kpini<lze; kpini++){
			...
			//PetscPrintf(PETSC_COMM_WORLD, "dpdcsi  = %.2f\n", dpdcsi[particle]);
			//PetscPrintf(PETSC_COMM_WORLD, "dpdx    = %.2f\n", dpdx[particle]);
			...
			Fpx[particle] = (0.25)*3.14*pow((diameter_particle),3)*dpdx[particle]*density_fluid*pow(v_scale,2);
			Fpy[particle] = (0.25)*3.14*pow((diameter_particle),3)*dpdy[particle]*density_fluid*pow(v_scale,2);
			Fpz[particle] = (0.25)*3.14*pow((diameter_particle),3)*dpdz[particle]*density_fluid*pow(v_scale,2);
			...
			acceleration_x[particle] = ((Fpx[particle] + Fbx[particle])/mass_particle)*(length_scale/pow(v_scale,2));
			acceleration_y[particle] = ((Fpy[particle] + Fby[particle])/mass_particle)*(length_scale/pow(v_scale,2));
			acceleration_z[particle] = ((Fpz[particle] + Fbz[particle])/mass_particle)*(length_scale/pow(v_scale,2));


                        // particle velocity (with mass)
			//uploc[particle] = uploc[particle] + acceleration_x[particle]*T_physical + random_vel_x;
			//vploc[particle] = vploc[particle] + acceleration_y[particle]*T_physical + random_vel_y;
			//wploc[particle] = wploc[particle] + acceleration_z[particle]*T_physical + random_vel_z;
...
*/
	...
	PetscFree(dpdcsi); PetscFree(dpdeta); PetscFree(dpdzet); PetscFree(dpdx); PetscFree(dpdy); PetscFree(dpdz); PetscFree(delp);
	PetscFree(Fpx); PetscFree(Fpy); PetscFree(Fpz);
	PetscFree(Fbx); PetscFree(Fby); PetscFree(Fbz);
...
PetscErrorCode Step_TimeIteration(UserCtx *user, PetscInt ti, PetscInt tis, PetscInt tsteps)
{
	...
	PetscReal   *Fpx, *Fpy, *Fpz,*Fp, *Fbx, *Fby, *Fbz;
	PetscReal   *acceleration_x, *acceleration_y, *acceleration_z;
	PetscReal   v_scale, density_particle, diameter_particle, density_fluid, mass_particle, T_physical, length_scale;
	...
	PetscFPrintf(PETSC_COMM_WORLD, fblock, "xploc,yploc,zploc,uploc,vploc,wploc,pressure,PG_x,PG_y,PG_z,PG_Force_x,PG_Force_y,PG_Force_z,Buoyancy_Force_x,Buoyancy_Force_y,Buoyancy_Force_z\n");//added pressure to print
	...
	for (bi=0; bi<block_number; bi++) {
		...
		for (particle=0; particle<NUM_OF_PARTICLE; particle++) {
			...
			*/
			...
			if (!interpolation) {
				// Snap to grid
				//uploc[particle] = ucat[kp[particle]][jp[particle]][ip[particle]].x;
				//vploc[particle] = ucat[kp[particle]][jp[particle]][ip[particle]].y;
				//wploc[particle] = ucat[kp[particle]][jp[particle]][ip[particle]].z;
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			
				...
				PetscReal xpite_forcast=0, ypite_forcast=0, zpite_forcast=0;
                        	...
                        	// Projection
                        	//PetscReal *vector_x,*vector_y,*vector_z;
                        	PetscReal vector_x=0,vector_y=0,vector_z=0;
                        	//vector_x[particle]  = xpite_forcast[particle] - X_centroid;
				//vector_y[particle]  = ypite_forcast[particle] - Y_centroid;
				//vector_z[particle]  = zpite_forcast[particle] - Z_centroid;
				...
				//PetscReal *normal_x,*normal_y,*normal_z,*distance,radius_pipe=0.05;
                        	...
                        	//distance = vector_x[particle]*normal_x + vector_y[particle]*normal_y + vector_z[particle]*normal_z;
                        	distance = vector_x*normal_x + vector_y*normal_y + vector_z*normal_z;
				...
				//Op_x = vector_x[particle] - Pp_x;
                        	//Op_y = vector_y[particle] - Pp_y;
                        	//Op_z = vector_z[particle] - Pp_z;
				...
				uploc[particle] = 0.03334 * ( 4*ucat[kp[particle]][jp[particle]][ip[particle]].x + ucat[kp[particle]][jp[particle]][ip[particle]-1].x + ucat[kp[particle]][jp[particle]][ip[particle]+1].x + ucat[kp[particle]-1][jp[particle]][ip[particle]].x + ucat[kp[particle]-1][jp[particle]][ip[particle]-1].x + ucat[kp[particle]-1][jp[particle]][ip[particle]+1].x + ucat[kp[particle]+1][jp[particle]][ip[particle]].x + ucat[kp[particle]+1][jp[particle]][ip[particle]-1].x + ucat[kp[particle]+1][jp[particle]][ip[particle]+1].x + ucat[kp[particle]][jp[particle]-1][ip[particle]].x + ucat[kp[particle]][jp[particle]-1][ip[particle]-1].x + ucat[kp[particle]][jp[particle]-1][ip[particle]+1].x + ucat[kp[particle]-1][jp[particle]-1][ip[particle]].x + ucat[kp[particle]-1][jp[particle]-1][ip[particle]-1].x + ucat[kp[particle]-1][jp[particle]-1][ip[particle]+1].x + ucat[kp[particle]+1][jp[particle]-1][ip[particle]].x + ucat[kp[particle]+1][jp[particle]-1][ip[particle]-1].x + ucat[kp[particle]+1][jp[particle]-1][ip[particle]+1].x + ucat[kp[particle]][jp[particle]+1][ip[particle]].x + ucat[kp[particle]][jp[particle]+1][ip[particle]-1].x + ucat[kp[particle]][jp[particle]+1][ip[particle]+1].x + ucat[kp[particle]-1][jp[particle]+1][ip[particle]].x + ucat[kp[particle]-1][jp[particle]+1][ip[particle]-1].x + ucat[kp[particle]-1][jp[particle]+1][ip[particle]+1].x + ucat[kp[particle]+1][jp[particle]+1][ip[particle]].x + ucat[kp[particle]+1][jp[particle]+1][ip[particle]-1].x + ucat[kp[particle]+1][jp[particle]+1][ip[particle]+1].x );

				vploc[particle] = 0.03334 * ( 4*ucat[kp[particle]][jp[particle]][ip[particle]].y + ucat[kp[particle]][jp[particle]][ip[particle]-1].y + ucat[kp[particle]][jp[particle]][ip[particle]+1].y + ucat[kp[particle]-1][jp[particle]][ip[particle]].y + ucat[kp[particle]-1][jp[particle]][ip[particle]-1].y + ucat[kp[particle]-1][jp[particle]][ip[particle]+1].y + ucat[kp[particle]+1][jp[particle]][ip[particle]].y + ucat[kp[particle]+1][jp[particle]][ip[particle]-1].y + ucat[kp[particle]+1][jp[particle]][ip[particle]+1].y + ucat[kp[particle]][jp[particle]-1][ip[particle]].y + ucat[kp[particle]][jp[particle]-1][ip[particle]-1].y + ucat[kp[particle]][jp[particle]-1][ip[particle]+1].y + ucat[kp[particle]-1][jp[particle]-1][ip[particle]].y + ucat[kp[particle]-1][jp[particle]-1][ip[particle]-1].y + ucat[kp[particle]-1][jp[particle]-1][ip[particle]+1].y + ucat[kp[particle]+1][jp[particle]-1][ip[particle]].y + ucat[kp[particle]+1][jp[particle]-1][ip[particle]-1].y + ucat[kp[particle]+1][jp[particle]-1][ip[particle]+1].y + ucat[kp[particle]][jp[particle]+1][ip[particle]].y + ucat[kp[particle]][jp[particle]+1][ip[particle]-1].y + ucat[kp[particle]][jp[particle]+1][ip[particle]+1].y + ucat[kp[particle]-1][jp[particle]+1][ip[particle]].y + ucat[kp[particle]-1][jp[particle]+1][ip[particle]-1].y + ucat[kp[particle]-1][jp[particle]+1][ip[particle]+1].y + ucat[kp[particle]+1][jp[particle]+1][ip[particle]].y + ucat[kp[particle]+1][jp[particle]+1][ip[particle]-1].y + ucat[kp[particle]+1][jp[particle]+1][ip[particle]+1].y );

				wploc[particle] = 0.03334 * ( 4*ucat[kp[particle]][jp[particle]][ip[particle]].z + ucat[kp[particle]][jp[particle]][ip[particle]-1].z + ucat[kp[particle]][jp[particle]][ip[particle]+1].z + ucat[kp[particle]-1][jp[particle]][ip[particle]].z + ucat[kp[particle]-1][jp[particle]][ip[particle]-1].z + ucat[kp[particle]-1][jp[particle]][ip[particle]+1].z + ucat[kp[particle]+1][jp[particle]][ip[particle]].z + ucat[kp[particle]+1][jp[particle]][ip[particle]-1].z + ucat[kp[particle]+1][jp[particle]][ip[particle]+1].z + ucat[kp[particle]][jp[particle]-1][ip[particle]].z + ucat[kp[particle]][jp[particle]-1][ip[particle]-1].z + ucat[kp[particle]][jp[particle]-1][ip[particle]+1].z + ucat[kp[particle]-1][jp[particle]-1][ip[particle]].z + ucat[kp[particle]-1][jp[particle]-1][ip[particle]-1].z + ucat[kp[particle]-1][jp[particle]-1][ip[particle]+1].z + ucat[kp[particle]+1][jp[particle]-1][ip[particle]].z + ucat[kp[particle]+1][jp[particle]-1][ip[particle]-1].z + ucat[kp[particle]+1][jp[particle]-1][ip[particle]+1].z + ucat[kp[particle]][jp[particle]+1][ip[particle]].z + ucat[kp[particle]][jp[particle]+1][ip[particle]-1].z + ucat[kp[particle]][jp[particle]+1][ip[particle]+1].z + ucat[kp[particle]-1][jp[particle]+1][ip[particle]].z + ucat[kp[particle]-1][jp[particle]+1][ip[particle]-1].z + ucat[kp[particle]-1][jp[particle]+1][ip[particle]+1].z + ucat[kp[particle]+1][jp[particle]+1][ip[particle]].z + ucat[kp[particle]+1][jp[particle]+1][ip[particle]-1].z + ucat[kp[particle]+1][jp[particle]+1][ip[particle]+1].z );

			}
				/*
				else {
					// Velocity interpolation by "TS13" Scheme
					//     Point 1:  ucat[k-1][j-1][i-1]
					//     Point 2:  ucat[k+1][j-1][i-1]
					//     Point 3:  ucat[k+1][j-1][i+1]
					//     Point 4:  ucat[k-1][j-1][i+1]
					//     Point 5:  ucat[k-1][j+1][i-1]
					//     Point 6:  ucat[k+1][j+1][i-1]
					//     Point 7:  ucat[k+1][j+1][i+1]
					//     Point 8:  ucat[k-1][j+1][i+1]
					//     Point 9:  ucat[k][j][i]
					//     Point 10: ucat[k-1][j][i]
					//     Point 11: ucat[k][j][i-1]
					//     Point 12: ucat[k+1][j][i]
					//     Point 13: ucat[k][j][i+1]
					uploc[particle]    = a1*ucat[kp[particle]-1][jp[particle]-1][ip[particle]-1].x + a2*ucat[kp[particle]+1][jp[particle]-1][ip[particle]-1].x + a3*ucat[kp[particle]+1][jp[particle]-1][ip[particle]+1].x + a4*ucat[kp[particle]-1][jp[particle]-1][ip[particle]+1].x + a5*ucat[kp[particle]-1][jp[particle]+1][ip[particle]-1].x + a6*ucat[kp[particle]+1][jp[particle]+1][ip[particle]-1].x + a7*ucat[kp[particle]+1][jp[particle]+1][ip[particle]+1].x + a8*ucat[kp[particle]-1][jp[particle]+1][ip[particle]+1].x + a9*ucat[kp[particle]][jp[particle]][ip[particle]].x + a10*ucat[kp[particle]-1][jp[particle]][ip[particle]].x + a11*ucat[kp[particle]][jp[particle]][ip[particle]-1].x + a12*ucat[kp[particle]+1][jp[particle]][ip[particle]].x + a13*ucat[kp[particle]][jp[particle]][ip[particle]+1].x;

					vploc[particle]    = a1*ucat[kp[particle]-1][jp[particle]-1][ip[particle]-1].y + a2*ucat[kp[particle]+1][jp[particle]-1][ip[particle]-1].y + a3*ucat[kp[particle]+1][jp[particle]-1][ip[particle]+1].y + a4*ucat[kp[particle]-1][jp[particle]-1][ip[particle]+1].y + a5*ucat[kp[particle]-1][jp[particle]+1][ip[particle]-1].y + a6*ucat[kp[particle]+1][jp[particle]+1][ip[particle]-1].y + a7*ucat[kp[particle]+1][jp[particle]+1][ip[particle]+1].y + a8*ucat[kp[particle]-1][jp[particle]+1][ip[particle]+1].y + a9*ucat[kp[particle]][jp[particle]][ip[particle]].y + a10*ucat[kp[particle]-1][jp[particle]][ip[particle]].y + a11*ucat[kp[particle]][jp[particle]][ip[particle]-1].y + a12*ucat[kp[particle]+1][jp[particle]][ip[particle]].y + a13*ucat[kp[particle]][jp[particle]][ip[particle]+1].y;

					wploc[particle]    = a1*ucat[kp[particle]-1][jp[particle]-1][ip[particle]-1].z + a2*ucat[kp[particle]+1][jp[particle]-1][ip[particle]-1].z + a3*ucat[kp[particle]+1][jp[particle]-1][ip[particle]+1].z + a4*ucat[kp[particle]-1][jp[particle]-1][ip[particle]+1].z + a5*ucat[kp[particle]-1][jp[particle]+1][ip[particle]-1].z + a6*ucat[kp[particle]+1][jp[particle]+1][ip[particle]-1].z + a7*ucat[kp[particle]+1][jp[particle]+1][ip[particle]+1].z + a8*ucat[kp[particle]-1][jp[particle]+1][ip[particle]+1].z + a9*ucat[kp[particle]][jp[particle]][ip[particle]].z + a10*ucat[kp[particle]-1][jp[particle]][ip[particle]].z + a11*ucat[kp[particle]][jp[particle]][ip[particle]-1].z + a12*ucat[kp[particle]+1][jp[particle]][ip[particle]].z + a13*ucat[kp[particle]][jp[particle]][ip[particle]+1].z;
				}
				*/
			...
			user[bi].Fbx[particle]  = Fbx[particle];
			user[bi].Fby[particle]  = Fby[particle];
			user[bi].Fbz[particle]  = Fbz[particle];

			PetscPrintf(PETSC_COMM_WORLD, "THIS IS THE PARTICLE NUMBER %d\n",particle);
			...
			PetscFPrintf(PETSC_COMM_WORLD, fblock, "%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e\n", xploc[particle], yploc[particle], zploc[particle], uploc[particle], vploc[particle], wploc[particle], presuure_ploc_x[particle], dpdx[particle], dpdy[particle], dpdz[particle], Fpx[particle], Fpy[particle], Fpz[particle], Fbx[particle], Fby[particle], Fbz[particle]);
PetscPrintf(PETSC_COMM_WORLD, "AFTER ITERATION \n");
		...
		if (ti==tie) {
			...
			PetscFree(user[bi].dpdcsi);
			PetscFree(user[bi].dpdeta);
			PetscFree(user[bi].dpdzet);
			...
			PetscFree(user[bi].Fbx);		
			PetscFree(user[bi].Fby);		
			PetscFree(user[bi].Fbz);		
...
//	PetscFree(presuure_ploc_x); PetscFree(Fpx); PetscFree(Fpy); PetscFree(Fpz); PetscFree(Fbx); PetscFree(Fby); PetscFree(Fbz);
